...
/* Subroutine */ int ft_pdlarfb(char *side, char *trans, char *direct, char *
	...
	side_len, ftnlen trans_len, ftnlen direct_len, ftnlen storev_len, t_checksuite *cs, t_Grid *grid, int* ReEntryin)
{
...
/*     Get grid parameters */
	...
	*(unsigned char *)uplo = 'L';
    ...
    infog2l_(iv, jv, &descv[1], &nprow, &npcol, &myrow, &mycol, &iiv, &jjv, &ivrow, &ivcol);
    infog2l_(ic, jc, &descc[1], &nprow, &npcol, &myrow, &mycol, &iic, &jjc, &icrow, &iccol);
    ldc = descc[9];
    ldv = descv[9];
    iic = min(iic,ldc);
    iiv = min(iiv,ldv);
    ...
    icoffv = (*jv - 1) % nbv;
    i__1 = *m + iroffc;
    mpc = numroc_(&i__1, &descc[5], &myrow, &icrow, &nprow);
    i__1 = *n + icoffc;
    nqc = numroc_(&i__1, &descc[6], &mycol, &iccol, &npcol);
    if (mycol == iccol) {
	nqc -= icoffc;
    }
    if (myrow == icrow) {
    ...
    if (lsame_(storev, "C", (ftnlen)1, (ftnlen)1)) 
	{
		...
		if (lsame_(side, "L", (ftnlen)1, (ftnlen)1)) 
		{
			...
			if (!ReEntry)
			{
				...
				pb_topget__(&ictxt, "Broadcast", "Rowwise", rowbtop, (ftnlen)9, (ftnlen)7, (ftnlen)1);
				if (mycol == ivcol) 
				{
					dgebs2d_(&ictxt, "Rowwise", rowbtop, &mpc, k, &v[ioffv], &ldv, (ftnlen)7, (ftnlen)1);
					if (myrow == ivrow) 
					{
						dtrbs2d_(&ictxt, "Rowwise", rowbtop, uplo, "Non unit", k, 
								k, &t[1], &nbv, (ftnlen)7, (ftnlen)1, (ftnlen)1, (ftnlen)8);
					}
					dlacpy_("All", &mpc, k, &v[ioffv], &ldv, &work[ipv], &lv, (ftnlen)3);
				} 
				else 
				{
					...
					if (myrow == ivrow) 
					{
						dtrbr2d_(&ictxt, "Rowwise", rowbtop, uplo, "Non unit", k, 
								k, &t[1], &nbv, &myrow, &ivcol, (ftnlen)7, (ftnlen)1, (ftnlen)1, (ftnlen)8);
					}
				}
			}
...
L10:
					if (*k - itop > 0) 
					{
						i__1 = iinxt - iibeg + 1;
						i__2 = *k - itop;
						dlaset_("Upper", &i__1, &i__2, &c_b21, &c_b22, &work[ipv 
			...
			{
				printf ("WARNING!!!! CASE NOT SUPPORTED\n");
...
L20:
				if (jj <= jjv + *k - 1) 
				{
					height = ibase - itop;
					i__1 = itop - jj + jjv;
					dlaset_("All", &kp, &i__1, &c_b21, &c_b21, &work[ipv + ii 
							- iiv + (jj - jjv) * lv], &lv, (ftnlen)3);
					dlaset_("Lower", &kp, &height, &c_b21, &c_b22, &work[ipv 
			...
			if (!ReEntry)
			{
				if (mpc > 0) 
				{
					dgemm_("Transpose", "No transpose", &nqc, k, &mpc, &c_b22, &
							c__[ioffc], &ldc, &work[ipv], &lv, &c_b21, &work[ipw],
							&lw, (ftnlen)9, (ftnlen)12);
				} 
				else 
				{
					dlaset_("All", &nqc, k, &c_b21, &c_b21, &work[ipw], &lw, (ftnlen)3);
				}
...
//#define INJECT
#ifdef INJECT
			int nb=grid->nb;
			real_err_step=err_block*nb*grid->npcol+nb*err_step+1;
			double *A = cs->kit.A;
			int *descA = &descv[1]; 
			...
			if (*jv==real_err_step)
			{
				...
				if (!ReEntry)
				{
					...
					{
						kill_proc (A, &descv[1], grid);

						memset (cs->kit.work, 0, cs->kit.lwork*sizeof(double));
						memset (cs->localcopy, 0, (cs->np_L)*(cs->nq_L)*sizeof(double));	
					}
					else
					{
						dump_to_disk (A, &descv[1], cs->kit.tau, cs->kit.work, cs->kit.lwork, grid, cs);

						// reconstruct V if needed
					}
				...
				{
					load_from_disk (A, &descv[1], cs->kit.tau, cs->kit.work, cs->kit.lwork, grid, cs);
					MPI_Barrier (MPI_COMM_WORLD);
			...
			if (!ReEntry)
			{
				if (myrow == ivrow) 
				{
					...
					dtrmm_("Right", uplo, transt, "Non unit", &nqc, k, &c_b22, &t[1], &nbv, // transt = 'N'
							&work[ipw], &lw, (ftnlen)5, (ftnlen)1, (ftnlen)1, (ftnlen)8);
			...
			if (!ReEntry)
			{
				dgemm_("No transpose", "Transpose", &mpc, &nqc, k, &c_b48, &work[ipv], &lv, 
						&work[ipw], &lw, &c_b22, &c__[ioffc], &ldc, (ftnlen)12, (ftnlen)9);
		...
		else 
		{
			...
			if (mycol == ivcol) {
				if (myrow == ivrow) {
					dlaset_("All", &iroffv, k, &c_b21, &c_b21, &work[ipw], &
							lw, (ftnlen)3);
					ipw1 = ipw + iroffv;
...
L30:
					if (*k - itop > 0) {
						i__1 = iinxt - iibeg + 1;
						i__2 = *k - itop;
						dlaset_("Upper", &i__1, &i__2, &c_b21, &c_b22, &work[
...
L40:
					if (jj <= jjv + *k - 1) {
						height = ibase - itop;
						i__1 = itop - jj + jjv;
						dlaset_("All", &kp, &i__1, &c_b21, &c_b21, &work[ipw1 
								+ ii - iiv + (jj - jjv) * lw], &lw, (ftnlen)3)
							;
						dlaset_("Lower", &kp, &height, &c_b21, &c_b22, &work[
			...
			pbdtran_(&ictxt, "Columnwise", "Transpose", &i__1, k, &mbv, &work[
					ipw], &lw, &c_b21, &work[ipv], &lv, &ivrow, &ivcol, &c_n1,
			...
			if (nqc > 0) {
				dgemm_("No transpose", "Transpose", &mpc, k, &nqc, &c_b22, &
						c__[ioffc], &ldc, &work[ipv], &lv, &c_b21, &work[ipw],
						&lw, (ftnlen)12, (ftnlen)9);
			} else {
				dlaset_("All", &mpc, k, &c_b21, &c_b21, &work[ipw], &lw, (
							ftnlen)3);
			}
			...
			if (mycol == ivcol) {
				...
				dtrmm_("Right", uplo, trans, "Non unit", &mpc, k, &c_b22, &t[
			...
			dgemm_("No transpose", "No transpose", &mpc, &nqc, k, &c_b48, &
					work[ipw], &lw, &work[ipv], &lv, &c_b22, &c__[ioffc], &
...
/*           WORK( IPT ) is the workspace for PBDTRAN */
	    ...
	    if (myrow == ivrow) {
		if (mycol == ivcol) {
		    dlaset_("All", k, &icoffv, &c_b21, &c_b21, &work[ipw], &
			    lw, (ftnlen)3);
		    ipw1 = ipw + icoffv * lw;
...
L50:
			...
			i__1 = *k - ileft;
			i__2 = jjnxt - jjbeg + 1;
			dlaset_("Lower", &i__1, &i__2, &c_b21, &c_b22, &work[
...
L60:
			...
			wide = iright - ileft;
			i__1 = ileft - ii + iiv;
			dlaset_("All", &i__1, &kq, &c_b21, &c_b21, &work[ipw1 
				+ ii - iiv + (jj - jjv) * lw], &lw, (ftnlen)3)
				;
			dlaset_("Upper", &wide, &kq, &c_b21, &c_b22, &work[
...
/*           WORK( IPV ) = WORK( IPW )' (replicated) is MPC0 x K */
	    ...
	    pbdtran_(&ictxt, "Rowwise", "Transpose", k, &i__1, &nbv, &work[
		    ipw], &lw, &c_b21, &work[ipv], &lv, &ivrow, &ivcol, &
...
/*           WORK( IPW ) = C( IOFFC )' * V'  (NQC x MPC x K) -> NQC x K */
		...
		dlaset_("All", &nqc, k, &c_b21, &c_b21, &work[ipw], &lw, (
			ftnlen)3);
	    }
...
/*                 Broadcast the block reflector to the other columns. */
		...
		dtrmm_("Right", uplo, transt, "Non unit", &nqc, k, &c_b22, &t[
...
/*                        MPC x NQC    MPC x K         K x NQC */
	    ...
	    dgemm_("No transpose", "Transpose", &mpc, &nqc, k, &c_b48, &work[
		    ipv], &lv, &work[ipw], &lw, &c_b22, &c__[ioffc], &ldc, (
		    ftnlen)12, (ftnlen)9);
...
L70:
		if (*k - ileft > 0) {
		    i__1 = *k - ileft;
		    i__2 = jjnxt - jjbeg + 1;
		    dlaset_("Lower", &i__1, &i__2, &c_b21, &c_b22, &work[ipv 
...
L80:
		if (ii <= iiv + *k - 1) {
		    wide = iright - ileft;
		    i__1 = ileft - ii + iiv;
		    dlaset_("All", &i__1, &kq, &c_b21, &c_b21, &work[ipv + ii 
			    - iiv + (jj - jjv) * lv], &lv, (ftnlen)3);
		    dlaset_("Upper", &wide, &kq, &c_b21, &c_b22, &work[ipv + 
